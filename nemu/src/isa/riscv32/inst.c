...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (imm + src1) & 0xfffffffe; IFDEF(CONFIG_FTRACE, FTRACE_RECORD));  // TODO     // pass
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = PRR(imm), printf("%x\n", PRR(imm)), PRW(imm, src1));
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = PRR(imm), PRW(imm, (PRR(imm) | src1)));

  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(11, cpu.pc));
