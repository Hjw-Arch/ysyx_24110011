...
static uint32_t charlist[] = {'*', '+', '-', '/', TK_EQ, TK_NEQ, TK_AND, TK_DEREF};
static uint32_t valuelist[] = {253, 254, 254, 253, 255, 255, 256, 252};
static uint32_t priority_list[300];

static void init_priority_list() {
    for (int i = 0; i < sizeof(charlist) / sizeof(charlist[0]); i++) {
        priority_list[charlist[i]] = valuelist[i];
    }
}
        ...
        *success = false;
        ...
        switch (tokens[pos_op].type)
        {
            ...
            case TK_DEREF: {
                ...
                if (!(val >= RAM_START_ADDR && val < RAM_END_ADDR)) {
                    *success = false;
                    printf("Out of memory range\n");
                    return 0;
                }
                uint32_t derefer_val = pmem_read(val, 4);  // 取该地址开始的连续四个字节内存数据
                return derefer_val;
            }
                break;
